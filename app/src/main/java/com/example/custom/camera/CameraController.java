...
package com.example.custom.camera;
...
import com.example.custom.AndroidUtilities;
import com.example.custom.ApplicationLoader;
import com.example.custom.Bitmaps;
import com.example.custom.SerializedData;
...
public class CameraController implements MediaRecorder.OnInfoListener {
    ...
    public void initCamera(final Runnable onInitRunnable) {
        ...
        threadPool.execute(() -> {
            try {
                if (cameraInfos == null) {
                    SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("test", Context.MODE_PRIVATE);
